...
class ProductRequest: CustomAlertView {
    ...
    static func getProductById(viewController vc: UIViewController, params: [String: Any], completion: @escaping ([ProductPrice]) -> ()) {
        Alamofire.request(ProductRouter.getProductById(params)).responseArray { (response: DataResponse<[ProductPrice]>) in
            switch response.result {
            case .success(let value):
                completion(value)
            case .failure(let error):
